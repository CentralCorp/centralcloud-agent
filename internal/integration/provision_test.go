...
func TestIsolatedDockerAndPostgresLifecycle(t *testing.T) {
	...
	spec := domain.ContainerSpec{Deployment: r, Environment: map[string]string{"PGPASSWORD_FILE": "/run/secrets/postgres_password"}, SecretFile: secret, ManagementLabels: deployment.ManagementLabels(r), TraefikLabels: deployment.TraefikLabels(r, c), FrontendNetwork: c.Docker.FrontendNetwork, EgressNetwork: c.Docker.EgressNetwork, User: "65532:65532", PidsLimit: 64}
